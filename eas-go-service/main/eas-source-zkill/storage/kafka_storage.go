...
import (
	...
	"eas-go-service/main/eas-source-zkill/model"
...
func (k *Kafka) Save(msg []byte) {
	...
	global.EASLog.Info("kafka producer send msg", zap.String("msg", kafkaMsgStr))
...
func convertToStr(value interface{}) (converted string) {
	...
	case reflect.String:
		global.EASLog.Warn("the string does not need to be converted, need update invoking")
		return value.(string)
	case reflect.Int:
	...
	default:
		global.EASLog.Error("unknown type convert to string", zap.Any("type", valueType.Kind()))
		return nilFlag
	}
}

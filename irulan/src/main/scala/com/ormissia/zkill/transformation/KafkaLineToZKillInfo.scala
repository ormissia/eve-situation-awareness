...
import com.ormissia.zkill.utils.{Character, ESAConst, ZKillInfo}
...
override def map(value: String): ZKillInfo = {
   ...
   val dateFormat = new SimpleDateFormat(ESAConst.DATE_FORMAT_yyyyMMddHHmmss)
   dateFormat.setTimeZone(TimeZone.getTimeZone("UTC"))
   val valueDt = dateFormat.format(new Date(values(0).toLong))
   // TODO log
   //println(s"${valueDt}-${values(0)}-${values(1)}-${values(2)}")
